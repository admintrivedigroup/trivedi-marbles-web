...
//
// Perspective-aware floor texture renderer for the combined visualizer test.
// Algorithm:
...
// Called once per render with the computed h and the floor quad.
...
// ── Main export: perspective floor render ────────────────────────────────────

const MAX_DIM = 1280;

export async function renderFloorPerspective(job: RenderJob): Promise<PerspectiveRenderResult> {
  ...
  return { compositeUrl, debugUrl, quad };
}
