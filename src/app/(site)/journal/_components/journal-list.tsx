...
function dateOf(post: JournalPost): string {
  return post.published_at ? formatBlogDate(post.published_at.slice(0, 10)) : "";
}
  ...
  if (posts.length === 0) {
    return (
      <div className="mx-auto max-w-screen-2xl px-6 py-32 text-center md:px-12 lg:px-24">
        <p className="font-serif text-2xl text-muted-foreground">No articles published yet.</p>
      </div>
    );
  }

  // If no article is explicitly featured, the newest published article
  // (posts are already sorted published_at desc) is used automatically.
  const featuredPost = posts.find((post) => post.is_featured) ?? posts[0];
  const rest = posts.filter((post) => post.id !== featuredPost.id);
  const gridPosts = rest.slice(0, visibleCount - 1);
  ...
  return (
    <>
      <section className="mx-auto max-w-screen-2xl px-6 py-24 md:px-12 lg:px-24">
        <FadeIn>
          <article className="flex flex-col items-center gap-12 lg:flex-row lg:gap-24">
            <div className="group w-full overflow-hidden lg:w-3/5">
              <Link href={`/journal/${featuredPost.slug}`} className="relative block aspect-video bg-gray-100 shadow-xl">
                <Image
            ...
            <div className="flex w-full flex-col lg:w-2/5">
              <span className="mb-4 text-sm uppercase tracking-[0.2em] text-secondary">
                {featuredPost.category} - {dateOf(featuredPost)}
              </span>
              <h2 className="mb-6 font-serif text-4xl leading-tight text-primary md:text-5xl">
                <Link href={`/journal/${featuredPost.slug}`} className="transition-colors hover:text-secondary">
                  {featuredPost.title}
                </Link>
              </h2>
              <p className="mb-10 text-lg leading-relaxed text-muted-foreground">{featuredPost.excerpt}</p>
              <Link
        ...
        <>
          ...
          <section className="mx-auto max-w-screen-2xl px-6 pb-16 md:px-12 lg:px-24">
            <FadeIn className="mb-16">
              <h3 className="font-serif text-3xl text-primary md:text-4xl">Latest Articles</h3>
            </FadeIn>
                  ...
                  <article className="group flex h-full flex-col">
                    ...
                    <div className="flex grow flex-col">
                      <span className="mb-3 text-xs uppercase tracking-[0.15em] text-secondary">
                        {post.category} - {dateOf(post)}
                      </span>
                      <h4 className="mb-4 font-serif text-2xl leading-tight text-primary">
                        <Link href={`/journal/${post.slug}`} className="transition-colors hover:text-secondary">
                          {post.title}
                        </Link>
                      </h4>
                      <p className="mb-6 grow text-sm leading-relaxed text-muted-foreground">{post.excerpt}</p>
                      <Link
                        href={`/journal/${post.slug}`}
              ...
              {hasMore ? (
                ...
                >
                  {isLoading ? (
                    ...
                    <>
                      Load More Articles
                      <span className="ml-1 text-xs opacity-60">({rest.length - gridPosts.length} remaining)</span>
                    </>
                  )}
                </button>
              ) : rest.length > INITIAL_VISIBLE - 1 ? (
                <p className="text-sm uppercase tracking-widest text-muted-foreground">You&apos;ve read them all</p>
              ) : null}
            </div>
          </section>
        </>
