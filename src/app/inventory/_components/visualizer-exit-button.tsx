...
// Shared by the standalone ExitButton and TopBar's Exit action — same
// animated close sequence, two different trigger UIs.
export function useExitTransition(href: string) {
  ...
  return { exiting, progress, triggerExit: () => setExiting(true) };
}

export function ExitOverlay({ progress }: { progress: number }) {
  return (
    <div
...
export function ExitButton({ href }: { href: string }) {
  const { exiting, progress, triggerExit } = useExitTransition(href);
