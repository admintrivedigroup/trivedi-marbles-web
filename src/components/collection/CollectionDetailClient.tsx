...
export default function CollectionDetailClient({
  ...
  const relatedMarbles = marbles.filter((item) => item.id !== marble.id).slice(0, 3);
  const imageOptions = [
    ...
    <>
      ...
      <div className="mb-24 grid grid-cols-1 gap-16 lg:grid-cols-2 lg:gap-24">
          ...
          <div className="hide-scrollbar flex gap-4 overflow-x-auto pb-4">
            {imageOptions.map((item, index) => (
              <button
                ...
                className={`flex w-24 flex-shrink-0 flex-col gap-2 text-left transition-all ${
        ...
        <FadeIn delay={0.2} className="flex flex-col justify-center">
          ...
          <button className="group flex w-full items-center justify-center gap-3 bg-primary py-5 text-sm uppercase tracking-widest text-white transition-colors duration-300 hover:bg-secondary">
      ...
      <FadeIn>
        ...
        <div className="grid grid-cols-1 gap-8 md:grid-cols-3">
          {relatedMarbles.map((item) => (
            <Link key={item.id} href={`/collection/${item.id}`} className="group block h-full">
